...
import (
	...
	"github.com/agentvoir/agentvoir/apps/registry-api/internal/policies"
...
func RegisterRoutes(mux *http.ServeMux, stores *Stores, tokenAccountingURL, opaURL string) {
	...
	budgets.NewHandler(stores.Budgets, tokenAccountingURL).RegisterRoutes(mux)
	modelroutes.NewHandler(stores.ModelRoutes).RegisterRoutes(mux)
	policies.NewHandler(opaURL).RegisterRoutes(mux)
